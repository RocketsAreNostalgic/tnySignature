...
PHPStan 2.2.14 uses the locked WordPress extension and stubs at **level 3** with
...
owns their reconciliation and removal, as well as the measured level-4/5
findings. Raising the level requires that work, not a broader suppression.
...
checks; they do **not** constitute a PHP product behavior suite. Real WordPress
activation, profile, settings and shortcode tests remain tracked in #5.

The WordPress 7.1 stubs describe analysis symbols, not a new minimum WordPress
version or evidence of WordPress 5.0 execution. The plugin's declared
**WordPress 5.0 / PHP 8.1** floors and existing PHPCS compatibility checks remain
